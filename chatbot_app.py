...
if not st.session_state.authenticated:
    ...
    try:
        ...
        st.error(
            f"Error accessing secrets. Make sure you've set up .streamlit/secrets.toml file")
...
with st.sidebar:
    ...
    if st.session_state.temperature < 0.1:
        st.warning(
            'Values approaching 0 produce deterministic outputs. The recommended starting value is 0.7')
    ...
    if not current_model_info['uses_top_p']:
        ...
        if st.session_state.top_p < 0.5:
            st.warning(
                'Low top_p values (<0.5) can make output more focused but less diverse. Recommended starting value is 0.9')
        if st.session_state.top_p == 1.0:
            st.info(
                'A top_p value of 1.0 means no nucleus sampling is applied (considers all tokens).')
    ...
    if min_tokens > 64:
        st.warning(
            f'{current_model} requires at least {min_tokens} input tokens.')
